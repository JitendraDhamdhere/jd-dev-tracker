  ...
  return (
    <>
      ...
      >
        ...
        {/* User Profile Footer */}
        <div className="p-3 border-t border-border-ui bg-bg-tertiary/40 flex items-center justify-between gap-1">
          <button
            onClick={() => onSelectTab("settings")}
            className="flex-1 flex items-center gap-2.5 p-1.5 rounded-xl hover:bg-bg-tertiary transition-colors text-left min-w-0"
          >
            <div className="w-9 h-9 rounded-xl bg-brand-primary font-heading font-bold text-white flex items-center justify-center text-xs shadow-md shrink-0">
              {profile.avatar || "JD"}
            </div>
            <div className="flex-1 min-w-0">
              <div className="font-semibold text-xs text-text-primary truncate">{profile.name}</div>
              <div className="text-[11px] text-text-muted truncate">{profile.role || "Software Developer"}</div>
            </div>
          </button>
          {onLogout && (
            <button
              onClick={onLogout}
              className="p-2 rounded-xl text-text-muted hover:text-status-danger hover:bg-bg-tertiary transition-colors shrink-0"
              title="Sign Out"
            >
              <LogOut size={16} />
            </button>
